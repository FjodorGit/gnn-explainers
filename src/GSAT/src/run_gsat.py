...
from GSAT.src.pretrain_clf import train_clf_one_seed
from GSAT.src.utils import Writer, Criterion, MLP, visualize_a_graph, save_checkpoint, load_checkpoint, get_preds, get_lr, set_seed, process_data
from GSAT.src.utils import get_local_config_name, get_model, get_data_loaders, write_stat_from_metric_dicts, reorder_like, init_metric_dict
...
class GSAT(nn.Module):
    ...
    def __loss__(self, att, clf_logits: torch.Tensor, clf_labels: torch.Tensor, epoch):
...
if __name__ == '__main__':
    ...
    local_config = yaml.safe_load((config_dir / local_config_name).open('r'))
    data_dir = Path('../../../data')
    log_dir = data_dir / dataset_name / 'logs' / (dataset_name + '-' + model_name + '-seed99-' + method_name + '-stat')
    train_gsat_one_seed(local_config, data_dir, log_dir, model_name, dataset_name , method_name, 'cpu', 41)
